...
MODES = {"ambiguity", "punctuation", "norm", "rewrite"}
...
def run():
    ...
    if by_expect.get("clean", 0) < 10:
        errors.append("слишком мало clean-примеров (нужно ≥10) — иначе не ловим выдуманное второе прочтение")
...
def test_coverage_complete():
    ...
    assert len(cases) >= 60, "ожидается не менее 60 примеров"
